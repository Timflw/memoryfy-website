...
var ze=Object.defineProperty;var Ee=(o,e,a)=>e in o?ze(o,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):o[e]=a;var d=(o,e,a)=>Ee(o,typeof e!="symbol"?e+"":e,a);var Ve=`#version 300 es
...
}`;function Ge(o){return"paperShaderMount"in o}function Te(){let o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function Ne(o){let e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,r=window.innerWidth-o.documentElement.clientWidth,u=e*a+r,n=outerWidth/u,c=Math.round(100*n);return c%5===0?c/100:c===33?1/3:c===67?2/3:c===133?4/3:n}var Qe={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},We={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Ye={none:0,contain:1,cover:2};var g=`
...
`;var le={maxColorCount:10,maxNoiseIterations:8},He=`#version 300 es
precision mediump float;
...
`,Je={circle:0,diamond:1,square:2,triangle:3};var he={maxColorCount:10},qe=`#version 300 es
precision mediump float;
...
`;var ce={maxColorCount:8,maxBallsCount:20},Ze=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${ce.maxBallsCount}; i++) {
    if (i >= int(ceil(u_count))) break;

    float idxFract = float(i) / float(${ce.maxBallsCount});
...
`;var xe={maxColorCount:5},eo=`#version 300 es
precision mediump float;
...
`,ao={checks:0,stripes:1,edge:2};var ye={maxColorCount:5},io=`#version 300 es
precision mediump float;
...
`;var Ae={maxColorCount:10},so=`#version 300 es
precision mediump float;
...
`,lo={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},co={random:1,"2x2":2,"4x4":3,"8x8":4};var be={maxColorCount:7},uo=`#version 300 es
...
`,fo={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7};var ue={maxColorCount:5,maxSpots:4},mo=`#version 300 es
precision lowp float;
...
void main() {
  ...
  for (int colorIdx = 0; colorIdx < ${ue.maxColorCount}; colorIdx++) {
    ...
    for (int spotIdx = 0; spotIdx < ${ue.maxSpots}; spotIdx++) {
  ...
  fragColor = vec4(color, opacity);
}`,po={auto:0,square:1};var fe={maxColorCount:7},go=`#version 300 es
precision lowp float;
...
void main() {
  ...
  vec4 premultipliedColors[${fe.maxColorCount}];
...
`;var Ce={maxColorCount:10},vo=`#version 300 es
precision mediump float;
...
`;var we={maxColorCount:10},ho=`#version 300 es
precision mediump float;
...
`,Ao={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},bo={prism:1,lens:2,contour:3,cascade:4,flat:5};var Co=`#version 300 es
precision mediump float;
...
`;var Re={maxSamples:50},wo=`#version 300 es
precision mediump float;
...
`;var Be={maxColorCount:10},So=`#version 300 es
...
`;function Bo(o){let e=document.createElement("canvas"),a=1e3;return new Promise((r,u)=>{let n=new Image;n.crossOrigin="anonymous",n.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(n.width=a,n.height=a);let c=n.naturalWidth/n.naturalHeight,t=Math.floor(a*.15),p=Math.ceil(t*2.5),s=a,v=a;c>1?v=Math.floor(a/c):s=Math.floor(a*c),e.width=s+2*p,e.height=v+2*p;let f=e.getContext("2d",{willReadFrequently:!0});if(!f)throw new Error("Failed to get canvas 2d context");f.fillStyle="white",f.fillRect(0,0,e.width,e.height),f.drawImage(n,p,p,s,v);let{width:m,height:F}=e,h=f.getImageData(0,0,m,F).data,i=m*F,l=new Uint8ClampedArray(i);for(let O=0;O<i;O++){let D=O*4,P=h[D]??0,j=h[D+1]??0,H=h[D+2]??0;l[O]=.299*P+.587*j+.114*H|0}let b=t,S=Math.max(1,Math.round(.12*t)),k=5,R=Se(l,m,F,b,3),w=Se(l,m,F,S,3),_=Se(l,m,F,k,1),M=f.createImageData(m,F),L=M.data;for(let O=0;O<i;O++){let D=O*4;L[D]=_[O]??0,L[D+1]=R[O]??0,L[D+2]=w[O]??0,L[D+3]=255}f.putImageData(M,0,0),e.toBlob(O=>{if(!O){u(new Error("Failed to create PNG blob"));return}r({blob:O})},"image/png")}),n.addEventListener("error",()=>{u(new Error("Failed to load image"))}),n.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Me(o,e,a,r){if(r<=0)return o.slice();let u=new Uint8ClampedArray(e*a),n=new Uint32Array(e*a);for(let c=0;c<a;c++){let t=0;for(let p=0;p<e;p++){let s=c*e+p,v=o[s]??0;t+=v,n[s]=t+(c>0?n[s-e]??0:0)}}for(let c=0;c<a;c++){let t=Math.max(0,c-r),p=Math.min(a-1,c+r);for(let s=0;s<e;s++){let v=Math.max(0,s-r),f=Math.min(e-1,s+r),m=p*e+f,F=p*e+(v-1),G=(t-1)*e+f,h=(t-1)*e+(v-1),i=n[m]??0,l=v>0?n[F]??0:0,b=t>0?n[G]??0:0,S=v>0&&t>0?n[h]??0:0,k=i-l-b+S,R=(f-v+1)*(p-t+1);u[c*e+s]=Math.round(k/R)}}return u}function Se(o,e,a,r,u){if(r<=0||u<=1)return Me(o,e,a,r);let n=o,c=o;for(let t=0;t<u;t++)c=Me(n,e,a,r),n=c;return c}var Uo=`#version 300 es
precision mediump float;
...
`,Z={measurePerformance:!1,workingSize:512,iterations:40};function Vo(o){let e=document.createElement("canvas"),a=e.getContext("2d"),r=typeof o=="string"&&o.startsWith("blob:");return new Promise((u,n)=>{if(!o||!a){n(new Error("Invalid file or canvas context"));return}let c=r&&fetch(o).then(s=>s.headers.get("Content-Type")),t=new Image;t.crossOrigin="anonymous";let p=performance.now();t.onload=async()=>{let s,v=await c;v?s=v==="image/svg+xml":typeof o=="string"?s=o.endsWith(".svg")||o.startsWith("data:image/svg+xml"):s=o.type==="image/svg+xml";let f=t.width||t.naturalWidth,m=t.height||t.naturalHeight;if(s){let y=f/m;f>m?(f=4096,m=4096/y):(m=4096,f=4096*y),t.width=f,t.height=m}let F=Math.min(f,m),h=Z.workingSize/F,i=Math.round(f*h),l=Math.round(m*h);Z.measurePerformance&&(console.log("[Processing Mode]"),console.log(`  Original: ${f}\xD7${m}`),console.log(`  Working: ${i}\xD7${l} (${(h*100).toFixed(1)}% scale)`),h<1&&console.log(`  Speedup: ~${Math.round(1/(h*h))}\xD7`)),e.width=f,e.height=m;let b=document.createElement("canvas");b.width=i,b.height=l;let S=b.getContext("2d");S.drawImage(t,0,0,i,l);let k=performance.now(),w=S.getImageData(0,0,i,l).data,_=new Uint8Array(i*l),M=new Uint8Array(i*l),L=0;for(let x=0,y=0;x<w.length;x+=4,y++){let z=w[x+3]===0?0:1;_[y]=z,L+=z}let O=[],D=[];for(let x=0;x<l;x++)for(let y=0;y<i;y++){let B=x*i+y;if(!_[B])continue;let z=!1;y===0||y===i-1||x===0||x===l-1?z=!0:z=!_[B-1]||!_[B+1]||!_[B-i]||!_[B+i]||!_[B-i-1]||!_[B-i+1]||!_[B+i-1]||!_[B+i+1],z?(M[B]=1,O.push(B)):D.push(B)}Z.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-k).toFixed(2)}ms`),console.log(`  Shape pixels: ${L} / ${i*l} (${(L/(i*l)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${D.length}`),console.log(`  Boundary pixels: ${O.length}`));let P=ko(_,M,new Uint32Array(D),new Uint32Array(O),i,l),j=performance.now(),H=Io(P,_,M,i,l);Z.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-j).toFixed(2)}ms`);let J=0,K;for(let x=0;x<D.length;x++){let y=D[x];H[y]>J&&(J=H[y])}let ee=document.createElement("canvas");ee.width=i,ee.height=l;let re=ee.getContext("2d"),W=re.createImageData(i,l);for(let x=0;x<l;x++)for(let y=0;y<i;y++){let B=x*i+y,z=B*4;if(!_[B])W.data[z]=255,W.data[z+1]=255,W.data[z+2]=255,W.data[z+3]=0;else{let te=255*(1-H[B]/J);W.data[z]=te,W.data[z+1]=te,W.data[z+2]=te,W.data[z+3]=255}}re.putImageData(W,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(ee,0,0,i,l,0,0,f,m);let N=a.getImageData(0,0,f,m),oe=document.createElement("canvas");oe.width=f,oe.height=m;let q=oe.getContext("2d");q.drawImage(t,0,0,f,m);let se=q.getImageData(0,0,f,m);for(let x=0;x<N.data.length;x+=4){let y=se.data[x+3],B=N.data[x+3];y===0?(N.data[x]=255,N.data[x+1]=0):(N.data[x]=B===0?0:N.data[x],N.data[x+1]=y),N.data[x+2]=255,N.data[x+3]=255}a.putImageData(N,0,0),K=N,e.toBlob(x=>{if(!x){n(new Error("Failed to create PNG blob"));return}if(Z.measurePerformance){let y=performance.now()-p;if(console.log(`[Total Processing Time] ${y.toFixed(2)}ms`),h<1){let B=y*Math.pow(f*m/(i*l),1.5);console.log(`[Estimated time at full resolution] ~${B.toFixed(0)}ms`),console.log(`[Time saved] ~${(B-y).toFixed(0)}ms (${Math.round(B/y)}\xD7 faster)`)}}u({imageData:K,pngBlob:x})},"image/png")},t.onerror=()=>n(new Error("Failed to load image")),t.src=typeof o=="string"?o:URL.createObjectURL(o)})}function ko(o,e,a,r,u,n){let c=a.length,t=new Int32Array(c*4);for(let p=0;p<c;p++){let s=a[p],v=s%u,f=Math.floor(s/u);t[p*4+0]=v<u-1&&o[s+1]?s+1:-1,t[p*4+1]=v>0&&o[s-1]?s-1:-1,t[p*4+2]=f>0&&o[s-u]?s-u:-1,t[p*4+3]=f<n-1&&o[s+u]?s+u:-1}return{interiorPixels:a,boundaryPixels:r,pixelCount:c,neighborIndices:t}}function Io(o,e,a,r,u){let n=Z.iterations,c=.01,t=new Float32Array(r*u),{interiorPixels:p,neighborIndices:s,pixelCount:v}=o,f=performance.now(),m=1.9,F=[],G=[];for(let h=0;h<v;h++){let i=p[h],l=i%r,b=Math.floor(i/r);(l+b)%2===0?F.push(h):G.push(h)}for(let h=0;h<n;h++){for(let i of F){let l=p[i],b=s[i*4+0],S=s[i*4+1],k=s[i*4+2],R=s[i*4+3],w=0;b>=0&&(w+=t[b]),S>=0&&(w+=t[S]),k>=0&&(w+=t[k]),R>=0&&(w+=t[R]);let _=(c+w)/4;t[l]=m*_+(1-m)*t[l]}for(let i of G){let l=p[i],b=s[i*4+0],S=s[i*4+1],k=s[i*4+2],R=s[i*4+3],w=0;b>=0&&(w+=t[b]),S>=0&&(w+=t[S]),k>=0&&(w+=t[k]),R>=0&&(w+=t[R]);let _=(c+w)/4;t[l]=m*_+(1-m)*t[l]}}if(Z.measurePerformance){let h=performance.now()-f;console.log(`[Optimized Poisson Solver (SOR \u03C9=${m})]`),console.log(`  Working size: ${r}\xD7${u}`),console.log(`  Iterations: ${n}`),console.log(`  Time: ${h.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${v}`),console.log(`  Speed: ${(n*v/(h*1e3)).toFixed(2)} Mpixels/sec`)}return t}var Fo={none:0,circle:1,daisy:2,diamond:3,metaballs:4};var Ro=`#version 300 es
precision mediump float;
...
`,Mo={classic:0,gooey:1,holes:2,soft:3},Oo={square:0,hex:1};var zo=`#version 300 es
precision mediump float;
...
`,Eo={dots:0,ink:1,sharp:2};var Ue={maxColorCount:6},Do=`#version 300 es
precision mediump float;
